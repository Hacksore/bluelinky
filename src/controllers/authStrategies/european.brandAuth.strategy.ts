...
export class EuropeanBrandAuthStrategy implements AuthStrategy {
  ...
  public async login(
    ...
    } = await got(this.environment.endpoints.integration, {
      cookieJar,
      json: true,
      headers: stdHeaders,
    });
    const brandAuthUrl = this.environment.brandAuthUrl({
      language: this.language,
      userId,
      serviceId,
    });
    const parsedBrandUrl = Url.parse(brandAuthUrl, true);
    const { body: authForm } = await got(brandAuthUrl, {
      cookieJar,
      headers: stdHeaders,
    });
    ...
    if (!redirectTo) {
      ...
      if (errorMessage) {
        throw new Error(
          `@EuropeanBrandAuthStrategy.login: Authentication failed with message : ${errorMessage[1]}`
        );
      }
      throw new Error(
        '@EuropeanBrandAuthStrategy.login: Authentication failed, cannot retrieve error message'
      );
    }
    ...
    if (!url) {
      throw new Error(
        `@EuropeanBrandAuthStrategy.login: after login redirection got stuck : ${htmlPage}`
      );
    }
    if (url.includes('login-actions/required-action')) {
      ...
      const actionUrl = loginActionUrl[1].startsWith('/')
        ? `${parsedBrandUrl.protocol}//${parsedBrandUrl.host}${loginActionUrl[1]}`
        : loginActionUrl[1];
      const loginActionForm = new URLSearchParams();
      loginActionForm.append('code', loginActionCode[1]);
      loginActionForm.append('accept', '');
      const {
        headers: { location: loginActionRedirect },
        body: AfterLoginActionAuthForm,
      } = await manageGot302(
        got.post(actionUrl, {
          cookieJar,
      ...
      if (!loginActionRedirect) {
        const errorMessage = /<span class="kc-feedback-text">(.+)<\/span>/gm.exec(
          AfterLoginActionAuthForm
        );
        if (errorMessage) {
          throw new Error(
            `@EuropeanBrandAuthStrategy.login: Authentication action failed with message : ${errorMessage[1]}`
          );
        }
        throw new Error(
          '@EuropeanBrandAuthStrategy.login: Authentication action failed, cannot retrieve error message'
        );
      }
    ...
    if (!appUser) {
      logger.warn(
        '@EuropeanBrandAuthStrategy.login: Cannot find the argument userId, please use the following url and connect from a browser.'
      );
    ...
    if (!body.redirectUrl) {
      throw new Error(
        `@EuropeanBrandAuthStrategy.login: silent sign In didn't work, could not retrieve auth code. status: ${statusCode}, body: ${JSON.stringify(
          body
        )}`
      );
    }
    ...
    if (!code) {
      throw new Error(
        `@EuropeanBrandAuthStrategy.login: Cannot find the argument code in ${body.redirectUrl}.`
      );
    }
    return {
      code: code as Code,
