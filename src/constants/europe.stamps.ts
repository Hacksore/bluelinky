...
const getAndCacheStampsFromFile = async (file: string, stampsFile = `https://raw.githubusercontent.com/neoPix/bluelinky-stamps/master/${file}.v2.json`): Promise<StampCollection> => {
...
export const getStampFromFile = (stampFileKey: string, stampsFile?: string) => async (): Promise<string> => {
    const { stamps, generated, frequency } = cachedStamps.get(stampFileKey) ?? await getAndCacheStampsFromFile(stampFileKey, stampsFile);
...
export const getStampGenerator = ({ mode, appId, brand, stampsFile }: { brand: Brand, mode: StampMode, appId: string, stampsFile?: string }): () => Promise<string> => {
    switch (mode) {
